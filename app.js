...
async function fetchWeatherData(location, date) {
    try {
        ...
        const apiKey = 'e45217111b64c2ba1f314bf9dfa0e2e1'; // Replace with your actual API key
...
// Only add listeners if feedback section exists (avoids errors)
if (feedbackYes && feedbackNo && submitFeedback) {
    let feedbackChoice = null;

    // Handle Yes/No button click
    feedbackYes.addEventListener('click', () => {
        feedbackChoice = 'Yes';
        feedbackYes.classList.add('active');
        feedbackNo.classList.remove('active');
    });

    feedbackNo.addEventListener('click', () => {
        feedbackChoice = 'No';
        feedbackNo.classList.add('active');
        feedbackYes.classList.remove('active');
    });

    // Handle feedback submission
    submitFeedback.addEventListener('click', () => {
        const userText = feedbackText.value.trim();

        if (!feedbackChoice && !userText) {
            feedbackMessage.style.color = 'var(--danger-color)';
            feedbackMessage.textContent = '⚠️ Please select Yes/No or write feedback!';
            return;
        }

        // Save feedback (currently just logs to console or localStorage)
        const feedbackData = {
            helpful: feedbackChoice || 'No answer',
            message: userText || 'No comment',
            timestamp: new Date().toISOString()
        };

        console.log('User Feedback:', feedbackData);

        // Store feedback locally (demo purpose)
        localStorage.setItem('paradecast_feedback', JSON.stringify(feedbackData));
...
function setTestDate(date) {
    ...
    // Try to use selected test location first
    const testSelect = document.getElementById('test-locations');
    const selected = testSelect ? testSelect.options[testSelect.selectedIndex] : null;
    ...
    if (selected && selected.value) {
        ...
        // If user typed a custom location
        handleSearch();
    } else {
